...
public class gAlchemistAI : MonoBehaviour {
    ...
    void MoveFrom(GameObject target)
    {
        ...
        if (!inRange)
        {
            ...
            if (this.transform.position.x <= target.transform.position.x - range)
            {
            ...
            else if (this.transform.position.x >= target.transform.position.x + range)
            {
    ...
    public void CreatePotion()
    {
        ...
        var newPotion = Instantiate(projectile, projectileSpawn.transform.position, Quaternion.identity);

        if (stats.aim.GetFacingRight())
        {

            newPotion.GetComponent<Rigidbody2D>().AddForce(new Vector2(1, 1) * throwForce);
